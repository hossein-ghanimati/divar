
import { createFormData, dynamicFieldsValidation, getSubDetails, handelCities, handelDynamicDefaultData, insertDynamicFields, sendFormData, statisFieldsValidation, handelNeighborhoods, handelPicUploader, mapMoveHandler } from "../../../utils/new/registerPost.js"
import { hideLoader, showLoader, showSwal } from "../../../utils/shared.js"

const registerPostBtn = document.querySelector('#register-btn')

const renderMap = () => {
  let map = L.map("map").setView([35.715298, 51.404343], 13)
  const firstIcon = L.icon({
    iconUrl: "data:image/svg+xml;base64,",
    iconSize: [30, 30]
  })
  const secondIcon = L.icon({
    iconUrl: "data:image/svg+xml;base64,",
    iconSize: [30, 30]
  })

  
  let markerIcon = firstIcon;
  let mapMarker = L.marker([35.715298, 51.404343], {icon: markerIcon}).addTo(map)

  L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png", {
    maxZoom: 19,
  }).addTo(map);

  map.on("move", () => {
    mapMoveHandler(map, mapMarker)
  })

  const checkbox = document.querySelector('.map-controll input')
  checkbox.addEventListener('change', e => {
    if (e.target.checked) {
      markerIcon = secondIcon      
      mapMarker.setIcon(markerIcon)
    }else{
      markerIcon = firstIcon
      mapMarker.setIcon(markerIcon)
    }
  })
}

const renderPictureSelecting = () => {
  handelPicUploader()
}

const renderCitySelecting = async () => {
  handelCities()
}

const loadSubDetails = async () => {
  const subDetails = await getSubDetails()
  console.log("Sub Details => ", subDetails);

  const categoryTitle = document.querySelector('.category_details p')
  categoryTitle.innerHTML = subDetails.title
  document.title = `ثبت آگهی - ${subDetails.title}`

  insertDynamicFields(subDetails.productFields)
  handelDynamicDefaultData(subDetails.productFields)
}

registerPostBtn.addEventListener('click', async () => {
  showLoader()
  const arePostFieldsValid = statisFieldsValidation() && dynamicFieldsValidation()
  
  if (arePostFieldsValid) {
    const formData = createFormData();
    const sendReq = await sendFormData(formData)
    if (sendReq.status <= 205) {
      showSwal('ساخت پست با موفقیت انجام شد.', 'پس از تایید مدیریت قابل نمایش است', "success", "باشه",  result => {
        if (result) {
          location.href = '../userPanel/posts.html'
        }
      })
    }else{
      showSwal('مشکلی پیش آمده', 'اینترنت و آیتم های ورودی را بررسی کنید.', 'error', 'باشه', () => null)
    }
  }else{
    showSwal('همه فیلد ها باید پر شوند..', null, 'warning', 'باشه', () =>  null)
  }
  hideLoader()
})

renderMap()
renderPictureSelecting()
loadSubDetails()
await renderCitySelecting()
hideLoader()