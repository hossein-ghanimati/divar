import { getToken, getUrlParam, mainURL } from "../../utils/shared.js";


let post = null;
let editedMapView = {};
let postEditedInfo = new FormData();
let postDynamicFields = []

const getPostData = async () => {
  const postID = getUrlParam('id')
  const getReq = await fetch(`${mainURL}/post/${postID}`)
  const response = await getReq.json()
  post = response.data.post
  return response.data
}

const renderMap = mapView => {
  const map = L.map("map-pre").setView([mapView.x, mapView.y], 13)
  const firstIcon = L.icon({
    iconUrl: "data:image/svg+xml;base64,",
    iconSize: [30, 30]
  })

  
  let markerIcon = firstIcon;
  let mapMarker = L.marker([mapView.x, mapView.y], {icon: markerIcon}).addTo(map)

  L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png", {
    maxZoom: 19,
  }).addTo(map);

  editedMapView = {...mapView}
}

const deletePost = async () => {
  const deleteReq = await fetch(`${mainURL}/post/${post._id}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${getToken()}`
    }
  })

  return deleteReq
}



////  edition \\\\\

const mapMoveHandler = (map, mapMarker) => {
  const center = map.getSize().divideBy(2);
  const targetPoint = map.containerPointToLayerPoint(center)
  const targetLatLng = map.layerPointToLatLng(targetPoint)

  mapMarker.setLatLng(targetLatLng)
  editedMapView.x = targetLatLng.lat;
  editedMapView.y = targetLatLng.lng
}

const renderEditMap = () => {

  const map = L.map("edit-map").setView([editedMapView.x, editedMapView.y], 10)
  const firstIcon = L.icon({
    iconUrl: "data:image/svg+xml;base64,",
    iconSize: [30, 30]
  })

  
  let markerIcon = firstIcon;
  let mapMarker = L.marker([editedMapView.x, editedMapView.y], {icon: markerIcon}).addTo(map)

  L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png", {
    maxZoom: 18,
  }).addTo(map);

  // let map = L.map("edit-map").setView([post.map.x, post.map.y], 13)
  // const firstIcon = L.icon({
  //   iconUrl: "data:image/svg+xml;base64,",
  //   iconSize: [30, 30]
  // })
  // const secondIcon = L.icon({
  //   iconUrl: "data:image/svg+xml;base64,",
  //   iconSize: [30, 30]
  // })

  
  // let markerIcon = firstIcon;

  // L.marker([post.map.x, (post.map.y)], {icon: secondIcon}).addTo(map)
  // // let mapMarker = L.marker([post.map.x, post.map.y], {icon: markerIcon}).addTo(map)

  // L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png", {
  //   maxZoom: 19,
  // }).addTo(map);

  // map.on("move", () => {
  //   mapMoveHandler(map, mapMarker)
  // })

}

const showPostFields = () => {
  const categoryTitleEl = document.querySelector('#category-title')
  categoryTitleEl.innerHTML = post.category.title

  renderEditMap()
}



export{
  getPostData,
  renderMap,
  deletePost,
  showPostFields
}